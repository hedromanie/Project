...
public:
    ...
    PrebuiltPacketIPv4(uint32_t src_ip, uint8_t* src_mac, uint32_t dst_ip, uint8_t* dst_mac,
        ...
        size_t min_size = sizeof(ether_header) + sizeof(ipv4_header) + sizeof(udphdr);
        ...
        udphdr* udp = (udphdr*)(buffer.data() + sizeof(ether_header) + sizeof(ipv4_header));
        udp->source = 0;
        ...
        if (payload_size) {
            uint8_t* payload = buffer.data() + sizeof(ether_header) + sizeof(ipv4_header) + sizeof(udphdr);
    ...
    void recalcChecksum() override {
        ...
        std::vector<uint8_t> pseudo_buf(sizeof(ps) + sizeof(udphdr) + payload_size);
        memcpy(pseudo_buf.data(), &ps, sizeof(ps));
        memcpy(pseudo_buf.data() + sizeof(ps), udp, sizeof(udphdr));
        if (payload_size) {
            uint8_t* payload = buffer.data() + sizeof(ether_header) + sizeof(ipv4_header) + sizeof(udphdr);
            memcpy(pseudo_buf.data() + sizeof(ps) + sizeof(udphdr), payload, payload_size);
        }
        udp->check = checksum((uint16_t*)pseudo_buf.data(), pseudo_buf.size());
    }
};
...
public:
    ...
    PrebuiltPacketIPv6(const uint8_t src_ip[16], uint8_t* src_mac, const uint8_t dst_ip[16],
        ...
        size_t min_size = sizeof(ether_header) + sizeof(ipv6_header) + sizeof(udphdr);
        payload_size = (packet_size > min_size) ? (packet_size - min_size) : 0;
        size_t total = min_size + payload_size;
        buffer.resize(total, 0);
        ...
        ip6->payload_len = htons(sizeof(udphdr) + payload_size);
        ip6->next_header = 17;
        ip6->hop_limit = 64;
        memcpy(ip6->saddr, src_ip, 16);
        memcpy(ip6->daddr, dst_ip, 16);

        udphdr* udp = (udphdr*)(buffer.data() + sizeof(ether_header) + sizeof(ipv6_header));
        udp->source = 0;
        udp->dest = htons(dst_port);
        udp->len = htons(sizeof(udphdr) + payload_size);
        udp->check = 0;
        ...
        if (payload_size) {
            uint8_t* payload = buffer.data() + sizeof(ether_header) + sizeof(ipv6_header) + sizeof(udphdr);
            memset(payload, 0, payload_size);
        }
    ...
    void recalcChecksum() override {
        ...
        ps.length = htonl(sizeof(udphdr) + payload_size);
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 6; i < argc; i++) {
        if (strcmp(argv[i], "--random-ip") == 0) random_ip = true;
            ...
            if (packet_size == 0) packet_size = 42; // минимальный для IPv4 UDP
        }
    ...
    if (packet_size == 0) {
        if (ipv6) packet_size = sizeof(ether_header) + sizeof(ipv6_header) + sizeof(udphdr);
        else      packet_size = sizeof(ether_header) + sizeof(ipv4_header) + sizeof(udphdr);
    }
